...
#include "models/detection_model_yolov4.h"
...
ModelYolo4::ModelYolo4(const std::string& modelFileName, float confidenceThreshold, bool useAutoResize,
...
void ModelYolo4::prepareInputsOutputs(InferenceEngine::CNNNetwork& cnnNetwork) {
    ...
    if (auto ngraphFunction = (cnnNetwork).getFunction()) {
        ...
        for (const auto op : ngraphFunction->get_ops()) {
            ...
            if (outputLayer != outputInfo.end()) {
                ...
                // slog::info << "op->get_shape():\t" << op->get_shape() << "\t" << op->get_shape().at(2) << "," << op->get_shape().at(3) << "\t"; 
                ...
                r.classes = 80;
                r.coords = 4;
                r.sides[0] = op->get_shape().at(2);
                r.sides[1] = op->get_shape().at(3);
                std::vector<float> anchors = {12.0, 16.0, 19.0, 36.0, 40.0, 28.0,
                                            36.0, 75.0, 76.0, 55.0, 72.0, 146.0,
                                            142.0, 110.0, 192.0, 243.0, 459.0, 401.0};

                std::vector<std::vector<int> > masks = {{6, 7, 8}, {0, 1, 2}, {3, 4, 5},};
                ...
                std::cout << r << "\t" << masks[maskCount].size() <<  " maskCount: " <<maskCount << std::endl;
                maskCount++;
...
std::unique_ptr<ResultBase> ModelYolo4::postprocess(InferenceResult & infResult) {
    ...
    for (auto& output : infResult.outputsData) {
        this->parseYOLOV4Output(output.first, output.second, netInputHeight, netInputWidth,
...
void ModelYolo4::parseYOLOV4Output(const std::string& output_name,
...
int ModelYolo4::calculateEntryIndex(int side, int lcoords, int lclasses, int location, int entry) {
    int n = location / (side * side);
    int loc = location % (side * side);
    return n * side * side * (lcoords + lclasses + 1) + entry * side * side + loc;
}

double ModelYolo4::intersectionOverUnion(const DetectedObject& o1, const DetectedObject& o2) {
